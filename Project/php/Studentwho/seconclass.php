...
<div class="container">    
  <h2><b>Students who got Second Class</b></h2>
    ...
    <tbody>
      <tr>
		...
		while($row1=mysqli_fetch_assoc($res1))
		{
			...
			$row3=mysqli_fetch_assoc($res3);
			$row4=mysqli_fetch_assoc($res4);
			 if(($row3['Assignment']+(($row3['IT1']+$row3['IT2'])/2)+$row4['TheoryMarks'])<75)
             {
				 
				 if(($row3['Assignment']+(($row3['IT1']+$row3['IT2'])/2)+$row4['TheoryMarks'])>=62.5)
				 {
			echo "<tr>";
